...
A etapa de elicitação de requisitos tem alta relevância no processo de desenvolvimento de sistemas, uma vez que é nela que se visa adquirir todas as informações necessárias para compreender, de forma clara e precisa, as reais necessidades dos usuários e demais envolvidos. Desse modo as expectativas em relação ao sistema são adquiridas, possibilitando a formatação dos requisitos funcionais e não funcionais.

Essa atividade é geralmente feita de modo interativo e, muitas vezes, utilizando várias estratégias para se adquirir um entendimento abrangente da situação. Entrevistas, questionários, \textit{workshops}, observações e sessões de \textit{brainstorming} são alguns exemplos das técnicas mais utilizadas. A escolha de uma determinada técnica depende do projeto, da disponiblidade dos participantes e do tipo de informação que será levantada.

Neste trabalho, optou-se por utilizar a técnica de \textit{brainstorming} como abordagem principal para a elicitação de requisitos. O \textit{brainstorming} é uma técnica criativa de geração de ideias que incentiva a participação ativa de todos os envolvidos, promovendo um ambiente livre de julgamentos e ideal para a coleta de sugestões espontâneas e diversas. Essa técnica mostrou-se adequada por permitir a rápida coleta de informações de diferentes pontos de vista, especialmente em um ambiente acadêmico com múltiplos atores, como alunos, professores e coordenadores.
...
\begin{itemize}
    ...
    \item \textbf{IA para reconhecimento de termo de compromisso:} Utilizar a IA para identificar termos de compromisso válidos
\end{itemize}
...
Após a etapa de elicitação, torna-se essencial organizar e classificar os requisitos identificados de acordo com sua importância e urgência. Essa priorização orienta o desenvolvimento incremental do sistema, garantindo que as funcionalidades mais críticas sejam implementadas primeiro, otimizando o uso de recursos e atendendo às expectativas principais dos \textit{stakeholders}

Neste trabalho, foi adotada a técnica \textbf{MoSCoW} para a priorização dos requisitos. Essa técnica, bastante utilizada em metodologias ágeis, categoriza os requisitos em quatro grupos principais: \textit{Must have}, \textit{Should have}, \textit{Could have} e \textit{Won’t have for now}.
